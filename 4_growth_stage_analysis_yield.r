################################################################################
# Phase 4: Growth Stage Analysis for Yield Prediction - TEMPORAL DATA VERSION
# 
# Purpose: Analyze yield prediction performance across growth stages
...
output_dir <- file.path(project_root, "soybean_analysis_outputs", "phase4_growth_stage_yield_temporal")
...
# Logging function
log_file <- file.path(output_dir, "phase4_growth_stage_yield_temporal_log.txt")
...
log_message("Starting Phase 4: Growth Stage Analysis for Yield - TEMPORAL VERSION")
...
prepare_stage_specific_temporal <- function(temporal_data, growth_stages = NULL, 
  ...
  stage_data <- stage_data[!is.na(stage_data$yield), ]
  
  log_message(sprintf("  Final dataset: %d plots, %d features", 
                      nrow(stage_data), ncol(stage_data) - 7))  # Adjusted for yield
...
create_performance_comparison_plot <- function(performance_data, output_file) {
  ...
  p <- ggplot(performance_data, aes(x = Stage, y = R2_mean, color = Model)) +
    ...
    labs(title = "Yield Prediction Performance with 95% Confidence Intervals (Temporal Data)",
...
sink(file.path(output_dir, "phase4_growth_stage_yield_temporal_summary.txt"))

cat("GROWTH STAGE ANALYSIS FOR YIELD - TEMPORAL DATA VERSION WITH CONFIDENCE INTERVALS\n")
cat("==================================================================================\n")
...
if (nrow(performance_summary_ci) > 0) {
  ...
  cat("1. BEST MODELS BY STAGE FOR YIELD (with 95% CI):\n")
  ...
  cat("\n3. INSIGHTS FOR YIELD PREDICTION:\n")
  cat("   - Using only spectral features (no LAI information)\n")
  cat("   - Features aggregated dynamically within growth stages\n")
  cat("   - No data leakage from pre-aggregated features\n")
  cat("   - Results show true predictive power of each growth stage\n")
}
...
log_message("\n=== Phase 4 Growth Stage Analysis for Yield Complete (Temporal Version) ===")
