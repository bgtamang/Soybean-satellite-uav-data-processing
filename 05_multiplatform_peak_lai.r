################################################################################
# Phase 5 Simplified: Multi-Platform Complementarity Analysis for Peak LAI Prediction
...
#
# Author: Simplified Multi-Platform Analysis for Peak LAI
# Date: 2024
################################################################################
...
phase5_dir <- file.path(project_root, "soybean_analysis_outputs", "phase5_multiplatform_complementarity_peak_lai")
...
log_file <- file.path(phase5_dir, "complementarity_peak_lai_analysis_log.txt")
...
log_message("Starting Simplified Multi-Platform Complementarity Analysis for PEAK LAI")
...
extract_features_by_platform <- function(data, platform = "both") {
  ...
  metadata_keywords <- c("DAP", "Pass", "Range", "Plot", "Location", "Folder", 
                         ...
                         "ProcessedFile", "Spacing", "ID", "LAI_TP", "LAI_DAP", "peak_lai")
...
stratified_sample_plots <- function(data, prop = 1.0, target_var = "peak_lai", n_strata = 5) {
...
analyze_complementarity <- function(train_data, test_data, features, n_bootstrap = N_BOOTSTRAP) {
  ...
  bootstrap_results <- foreach(b = 1:n_bootstrap,
    ...
    boot_idx <- stratified_sample_plots(train_data, 1.0, "peak_lai")
...
plot_complementarity_results <- function(comp_results, stat_tests, location, output_dir) {
  ...
  pdf(file.path(output_dir, paste0(location, "_complementarity_peak_lai_analysis.pdf")), 
      width = 16, height = 12)
...
plot_feature_importance <- function(importance_results, location, output_dir) {
  ...
  pdf(file.path(output_dir, paste0(location, "_feature_importance_peak_lai.pdf")), 
      width = 14, height = 10)
...
for (location in names(datasets_aligned)) {
  ...
  saveRDS(results, file.path(phase5_dir, "model_performance", 
                            sprintf("%s_complementarity_results_peak_lai.rds", location)))
...
write_csv(summary_data, file.path(phase5_dir, "summary_tables", 
                                  "platform_comparison_summary_peak_lai.csv"))
...
# Create text report
sink(file.path(phase5_dir, "complementarity_analysis_summary_report_peak_lai.txt"))

cat("MULTI-PLATFORM COMPLEMENTARITY ANALYSIS FOR PEAK LAI - SUMMARY\n")
cat("==============================================================\n")
...
log_message("\n=== SIMPLIFIED MULTI-PLATFORM COMPLEMENTARITY ANALYSIS FOR PEAK LAI COMPLETE ===", log_time = TRUE)
