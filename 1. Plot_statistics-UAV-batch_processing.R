...
#----------------------------------------------------------------
# PROCESS SINGLE FOLDER FUNCTION
#----------------------------------------------------------------
process_single_uav_folder <- function(base_dir, field, date_folder_name) {
  # Create log file
  log_file <- file.path(base_dir, paste0("processing_log_", field, "_", date_folder_name, ".txt"))
  ...
  # Path to UAV folder
  uav_path <- file.path(field_path, "UAV")
  if (!dir.exists(uav_path)) {
    error_msg <- paste("UAV directory not found for field:", field)
    message(error_msg)
    cat(paste(error_msg, "\n"), file=log_file, append=TRUE)
    return(FALSE)
  }
  ...
  date_folder <- file.path(uav_path, date_folder_name)
  ...
  # Process the folder
  message(paste("Processing folder:", date_folder_name))
  cat(paste("\nProcessing folder:", date_folder_name, "at", Sys.time(), "\n"), file=log_file, append=TRUE)
  
  # Find the necessary files in this folder
  tryCatch({
    ms_files <- list.files(date_folder, pattern=".*data\\.tif$|.*MS\\.tif$|.*_ms_.*\\.tif$", 
    ...
    if (length(ms_files) == 0) {
      error_msg <- paste("No MS file found in:", date_folder)
      message(error_msg)
      cat(paste(error_msg, "\n"), file=log_file, append=TRUE)
      return(FALSE)
    }
    ...
    # Log the files found
    cat(paste("MS file:", basename(ms_path), "\n"), file=log_file, append=TRUE)
    cat(paste("Shapefile:", basename(shapefile_path), "\n"), file=log_file, append=TRUE)
    cat(paste("Output folder:", output_folder, "\n"), file=log_file, append=TRUE)
    
    # Process the files
    message("Processing files...")
    results <- process_plots_r(ms_path, shapefile_path, output_folder)
    
    # Log success
    success_msg <- paste("Successfully processed folder:", date_folder_name)
    ...
    error_msg <- paste("Error processing folder:", date_folder, "- Error:", conditionMessage(e))
    message(error_msg)
    cat(paste(error_msg, "\n"), file=log_file, append=TRUE)
    return(FALSE)
  })
...
#----------------------------------------------------------------
# MS PROCESSING FUNCTIONS
#----------------------------------------------------------------

# Function to process plot data with MS orthomosaic only
process_plots_r <- function(ms_path, shapefile_path, output_folder) {
  # Create output directory if it doesn't exist
  if (!dir.exists(output_folder)) dir.create(output_folder, recursive = TRUE)
  
  # Read raster data
  ms_raster <- rast(ms_path)
  ...
  cat(sprintf("MS raster has %d bands\n", nlyr(ms_raster)))
  cat("MS raster band names:", names(ms_raster), "\n")
  ...
  for (i in 1:nrow(plot_polygons)) {
    ...
    # Extract MS values
    tryCatch({
      ms_vals <- terra::extract(ms_raster, plot_polygons[i,])
      # Remove the ID column
      ms_vals <- ms_vals[, -1, drop = FALSE]
      
      # Calculate MS statistics for each band
      for (band_idx in 1:ncol(ms_vals)) {
        band_name <- names(ms_raster)[band_idx]
        band_vals <- ms_vals[, band_idx]
        
        # Clean values
        band_vals <- band_vals[!is.na(band_vals) & band_vals > 0]
        
        # Store the cleaned band values for index calculation
        clean_name <- gsub(" ", "_", band_name)  # Normalize band name
        all_plot_pixels[[i]]$MS[[clean_name]] <- band_vals
        
        if (length(band_vals) > 0) {
          # Add statistics to raw_data with consistent column names
          clean_name <- paste0("MS_", clean_name)
          raw_data[i, paste0(clean_name, "_Mean")] <- mean(band_vals)
          raw_data[i, paste0(clean_name, "_Median")] <- median(band_vals)
          raw_data[i, paste0(clean_name, "_Sum")] <- sum(band_vals)
          raw_data[i, paste0(clean_name, "_Sum_per_m2")] <- sum(band_vals) / plot_areas[i]
          # Add pixel count
          raw_data[i, paste0(clean_name, "_PixelCount")] <- length(band_vals)
        }
      }
    }, error = function(e) {
      cat(sprintf("Error processing MS data for plot %s: %s\n", plot_id, e$message))
    })
  }
  
  # Calculate vegetation indices at the pixel level
  cat("Calculating vegetation indices at pixel level...\n")
  raw_data <- calculate_vegetation_indices(raw_data, all_plot_pixels, output_folder)
...
# Function to calculate vegetation indices at the pixel level
calculate_vegetation_indices <- function(df, all_plot_pixels, output_folder) {
  # Print all column names to debug
  cat("Available column names in dataset:\n")
  cat(paste(names(df), collapse=", "), "\n\n")
  
  # Check if MS platform is available
  if (!any(grepl("^MS_", names(df)))) {
    cat("MS platform not available. No indices to calculate.\n")
    return(df)
  }
  
  cat("MS platform is available\n")
  
  # Function to check if all bands exist for a specific platform in a plot's data
  bands_exist_in_plot <- function(plot_data, platform, band_list) {
    all(band_list %in% names(plot_data[[platform]]))
  }
  
  # List of vegetation indices for MS platform only (with available bands: Red, Green, NIR, Red_edge)
  indices <- list(
    "NDVI" = list(
  ...
  for (index_name in names(indices)) {
    ...
    # Create index column name
    index_column_name <- sprintf("%s_MS", index_name)
    cat(sprintf("Calculating %s using MS bands at pixel level...\n", index_column_name))
    ...
    for (plot_idx in 1:length(all_plot_pixels)) {
      ...
      if (!bands_exist_in_plot(plot_data, "MS", index_info$bands)) {
      ...
      for (band in index_info$bands) {
        band_values[[band]] <- plot_data[["MS"]][[band]]
      }
  ...
  cat("\nSuccessfully calculated MS indices:\n")
  success_indices <- index_summary[grepl("Successfully", index_summary$Status), ]
  if (nrow(success_indices) > 0) {
    cat(paste("  ", success_indices$Index, collapse = "\n  "), "\n")
  }
...
detect_and_completely_remove_outliers <- function(df, output_folder) {
  ...
  index_pattern <- "^[A-Za-z0-9]+_MS(_Mean|_Median|_Sum)?$"
...
#----------------------------------------------------------------
# AUTOMATIC BATCH PROCESSING FUNCTION
#----------------------------------------------------------------

# Function to get all UAV folders
get_all_uav_folders <- function(base_dir) {
  folder_list <- list()
  ...
  for (field in c("EF", "SF")) {
    ...
    uav_path <- file.path(field_path, "UAV")
    if (!dir.exists(uav_path)) {
      message(paste("UAV directory not found for field:", field))
      next
    }
    
    date_folders <- list.dirs(uav_path, full.names = FALSE, recursive = FALSE)
...
process_all_uav_folders_sequentially <- function(base_dir = getwd()) {
  # Create main log file
  master_log <- file.path(base_dir, "batch_processing_master_log.txt")
  cat(paste("UAV Batch Processing started at:", Sys.time(), "\n"), file = master_log)
  
  # Get all UAV folders to process
  folders <- get_all_uav_folders(base_dir)
  
  if (length(folders) == 0) {
    message("No UAV folders found to process.")
    cat("No UAV folders found to process.\n", file = master_log, append = TRUE)
    return()
  }
  
  cat(sprintf("Found %d UAV folders to process.\n", length(folders)), file = master_log, append = TRUE)
  ...
  for (i in seq_along(folders)) {
    ...
    message(sprintf("\nProcessing folder %d of %d: %s/%s", i, length(folders), field, folder))
    cat(sprintf("\nProcessing folder %d of %d: %s/%s at %s\n", 
    ...
    # Process the folder
    result <- tryCatch({
      output <- process_single_uav_folder(base_dir, field, folder)
    ...
    # Log result
    result_msg <- sprintf("Folder %s/%s processed %s in %.2f minutes", 
                          field, folder, 
    ...
    summary_path <- file.path(base_dir, "batch_processing_summary.csv")
...
# Run the batch processing
cat("\n========== STARTING UAV BATCH PROCESSING ==========\n")
process_all_uav_folders_sequentially(base_dir)
cat("\n========== UAV BATCH PROCESSING COMPLETE ==========\n")